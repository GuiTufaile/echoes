...
// 	
// 	Customizado por Gabriel A. Fernandes em Janeiro / 2016
//
...
class classMySQL {
	...
	function abrir_conexao() {
		...
		$this->conexao = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME) or
	...
	function inserir_dados($tabela, $dados){
			...
			$this->abrir_conexao();
			$this->dados = mysqli_query($this->conexao, $sql) or die(mysqli_error($this->conexao));
			$ult_inserido = mysqli_insert_id($this->conexao);
			$this->fechar_conexao();
			return $ult_inserido;
